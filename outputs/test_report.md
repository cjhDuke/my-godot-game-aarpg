...
- 人工测试补充：用户已手动测试，发现 projectile 碰到墙 TileMap 时不会自动销毁；后续新测试又发现 dungeon 中 MageEnemy 和普通 Goblin 位于 y 为负的位置时不会显示，但功能仍可正常实现，初步怀疑是 dungeon TileMap 或场景可视配置问题；x 为负貌似没有问题；其余原本需要人工确认的条目暂未发现问题。
...
| A2-4 | 新增 MageEnemy 后普通 Enemy 仍然可以正常运行 | FAIL | Goblin/Slime 场景可加载、可实例化，`playground.tscn` 可运行；但用户人工测试发现 dungeon 中普通 Goblin 位于 y 为负的位置时不会显示，功能仍正常；x 为负貌似没有问题。该问题疑似 dungeon 场景/TileMap 配置问题，并非本次 MageEnemy 改动直接导致。 |
...
| A3-2 | MageEnemy 可以正常显示 | FAIL | 用户人工测试发现 dungeon 中 MageEnemy 位于 y 为负的位置时不会显示，但功能仍正常；x 为负貌似没有问题。该问题疑似 dungeon 场景/TileMap 配置问题，并非本次 MageEnemy 改动直接导致。 |
...
| A7-5 | projectile 命中墙体或障碍物后会销毁 | FAIL | 用户人工测试发现 projectile 碰到墙 TileMap 时不会自动销毁；自动化仅直接调用了 body collision 路径，未覆盖真实 TileMap 碰撞。 |
...
| A2-4 / A3-2 | dungeon 中 y 为负时敌人不显示 | 用户人工测试发现 MageEnemy 和普通 Goblin 在 dungeon 中位于 y 为负的位置时不会显示，但功能仍可正常实现；x 为负貌似没有问题；初步怀疑是 dungeon TileMap 或场景可视配置问题。 |

自动化运行时断言结果为 44 PASS / 0 FAIL；人工测试累计新增 2 类 FAIL。
...
4. 观察结果：projectile 碰到墙 TileMap 后不会自动销毁。
...
- A7-5 失败可能与 projectile 真实运行时未收到 TileMap 的 `body_entered` 信号有关。
- 也可能是 TileMap 碰撞层、TileMapLayer 类型、Area2D collision mask/monitoring，或 projectile 与墙体碰撞形状交互方式不匹配。
- 自动化测试曾直接调用 body collision 路径，因此未覆盖真实 TileMap 物理碰撞，导致该问题需要人工测试才能发现。
- dungeon 中 y 为负时敌人不显示，同时影响 MageEnemy 和普通 Goblin，且功能仍正常；x 为负貌似没有问题。该现象说明问题更可能在 dungeon TileMap、CanvasItem 可见性、YSort/层级、Camera/viewport、遮挡或场景区域配置，而不是 MageEnemy 专属逻辑。

## 10. 修复建议

- 暂时不要修复功能代码。
- 后续进入 Phase 5 时，优先修复 A7-5：确认 projectile 与 TileMap/墙体的真实碰撞信号，必要时改用更可靠的碰撞检测方式。
- dungeon 中 y 为负时敌人不显示的问题也暂时不要修复；后续可单独检查 dungeon TileMap、渲染层级、YSort、遮挡层、Camera 边界和相关场景节点配置。
...
11. 当前已知失败项：让 projectile 撞墙 TileMap，观察到 projectile 不会自动销毁。
12. 当前新增已知失败项：在 dungeon 中将 MageEnemy 或普通 Goblin 放到 y 为负的位置，观察到敌人不显示，但功能仍正常；x 为负貌似没有问题。
...
人工测试补充结论：FAIL。用户发现 A7-5 不通过：projectile 碰到墙 TileMap 时不会自动销毁；同时发现 dungeon 中 MageEnemy 和普通 Goblin 位于 y 为负的位置时不会显示，但功能仍正常；x 为负貌似没有问题。

完整验收结论：FAIL。当前无 BLOCKED 项；已知失败包括 projectile 碰墙 TileMap 不销毁，以及 dungeon 中 y 为负时 MageEnemy/普通 Goblin 不显示。根据用户要求，本阶段只记录测试结果，暂不修复。
