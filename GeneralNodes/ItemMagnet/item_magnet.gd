...
func _exit_tree() -> void:
	for i in range(items.size() - 1, -1, -1):
		...
		if _item == null:
			continue
		if _item.collect() == false:
			_item.set_physics_process(true)
	items.clear()
	speeds.clear()
...
func _process(delta: float) -> void:
	for i in range(items.size() - 1, -1, -1):
		...
		if _item == null:
			...
			_item.global_position += _item.global_position.direction_to(global_position) * speeds[i]
		else:
			_item.global_position = global_position
			if _item.collect() == true:
				items.remove_at(i)
				speeds.remove_at(i)
